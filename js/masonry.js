(function () {
	...
	function createMasonryItem(item) {
		...
		img.loading = 'lazy';
		img.src = normalizedItem.src;
		img.alt = normalizedItem.alt || '';
		figure.appendChild(img);

		if (normalizedItem.caption) {
			const figcaption = document.createElement('figcaption');
			figcaption.textContent = normalizedItem.caption;
	...
	function layoutMasonryWall(wall, columnCount) {
		...
		const measureItemHeight = (item) => {
			...
			if (!image) return 220;
			return image.naturalHeight ? image.naturalHeight / (image.naturalWidth / 320) : 220;
		};

		items.forEach((item) => {
			const itemHeight = measureItemHeight(item);
	...
	function renderMasonryWalls(containerId = 'masonry-sections') {
		...
		masonrySections.forEach((section) => {
			...
			layoutMasonryWall(wall, columnCount);
	...
	document.addEventListener('DOMContentLoaded', () => {
		...
		window.addEventListener('resize', () => renderMasonryWalls());
		window.addEventListener('load', () => renderMasonryWalls());
	});
